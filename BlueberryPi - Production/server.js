...
async function initialize() {
    ...
    app.listen(PORT || 3000, () => console.log(`✅ Proxy running on http://localhost:${PORT || 3000}`));
    udpServer.bind(9090, () => console.log('✅ UDP server listening on port 9090'));
...
process.on('uncaughtException', (err) => {
    if (err.code === 'EPIPE') {
        console.warn("Global EPIPE caught. Likely writing to closed stream.");
    } else {
...
wsServer.on('connection', (ws, req) => {
    ...
    ws.on('close', () => console.log(`Client IP: ${clientIP} disconnected`));
});
...
function handleWsMessage(ws, data) {
    ...
    if (!data || data.length < prefixLength) {
        console.warn("Invalid or too short data received");
        return;
    }
    ...
    try {
        ...
        switch (header) {
            ...
            case "WEB_":
                webClients.push(ws);
                console.log("Web client added");
                break;
            case "ESP3":
                console.log("ESP32 client added");
                break;
            case "PONG":
                ...
                if (!isNaN(sentTimestamp)) {
                    ...
                    console.warn("Invalid timestamp in PONG message");
                    console.log(data.toString('utf8').slice(prefixLength));
                }
                break;
            default:
                console.log("Unhandled message:", data.toString("utf8"));
                break;
        }
    } catch (err) {
        console.error("Error in handleWsMessage:", err.stack || err);
    }
}
            ...
            if (!client.lastSent || now - client.lastSent >= interval) {
                for (const camId in cameras) {
                    ...
                    if (cam.latestFrame) {
                        ...
                        // Every 10 seconds, send a PONG frame with timestamp
                        if (now - lastPongSent >= PONG_INTERVAL_MS) {
                            ...
                            // console.log("➡️ Sending PONG to client", client._socket.remoteAddress, client.latency);
                        ...
                        try {
                            if (client.readyState === WebSocket.OPEN) {
                                client.send(framedData);
                                client.lastSent = now;
                            }
                        } catch (err) {
                            if (err.code === 'EPIPE') {
                                console.warn("EPIPE during WebSocket send. Client likely disconnected.");
                            } else {
...
setInterval(() => {
  ...
  for (const [identifier, cam] of Object.entries(cameras)) {
    ...
    if (now - cam.lastFrameTime > TIMEOUT_MS) {
      console.warn(`${identifier}: No frames received for ${TIMEOUT_MS/1000}s, closing FFmpeg...`);
      try {
        cam.stream.end();
      } catch {}
      try {
        cam.ffmpegProcess.stdin.end();
      } catch (e) {
        console.error(`${identifier}: Error closing FFmpeg stdin:`, e);
      }
    }
  }
}, 1000);
...
async function handleStatusUpdate(status) {
    ...
    if (temperature !== prevTemp && camId) {
        ...
        try {
            ...
            console.log(`Temperature ${temperature} for ${camId} logged in database.`);
...
function processVideoData(data) {
    ...
    if (validJPEG) {
        ...
        if (encodingShutdown) {
            ...
            return;
        } else if (currCamera.encodingRestarting) {
            ...
            try {
                ...
                if (err.code === 'EPIPE') {
                    console.warn(`Caught EPIPE while writing to ${cameraIdentifier}. Restarting encoding.`);
                    // restartEncoding(cameraIdentifier);
                } else {
                    console.error(`Unexpected error while writing to stream:`, err);
                }
            }
...
function startEncoding(identifier) {
    ...
    try {
        ...
        console.warn(`${identifier}: Base directory not writable, using backup.`);
        activePath = SAVE_FILEPATH_BACK;
    }

    const filenamePattern = path.join(
        activePath,
    ...
    stream.on('error', (err) => {
        if (err.code === 'EPIPE') {
            console.warn(`EPIPE caught on stream for ${identifier}. Restarting encoding.`);
            // restartEncoding(identifier);
        } else {
            console.error(`Stream error for ${identifier}:`, err);
        }
    });

    console.log(`Starting FFmpeg stream: ${filenamePattern}`);
    ...
    ffmpegProcess.stdout.on("data", (data) => {
        ...
        if (!output.includes("continue")) {
            console.log(`FFmpeg Progress (${identifier}): ${output}`);
        }
    });
...
/**
 * Restarts the encoding process for a specific camera.
 * @param {string} identifier The camera identifier.
 */
...
//     try {
//         // Gracefully end the current stream and FFmpeg process
//         if (camera.stream) {
//             camera.stream.unpipe(camera.ffmpegProcess.stdin);
//             camera.stream.end();
//         }

//         if (camera.ffmpegProcess?.stdin && !camera.ffmpegProcess.stdin.destroyed) {
//             camera.ffmpegProcess.stdin.end();
//         }

//         if (camera.ffmpegProcess && !camera.ffmpegProcess.killed) {
//             camera.ffmpegProcess.kill('SIGTERM');
//         }
//     } catch (err) {
//         console.error(`Error during shutdown of ${identifier}:`, err);
//     }

//     // Wait briefly before restarting
//     setTimeout(() => {
//         const newCamera = startEncoding(identifier);
//         newCamera.encodingRestarting = false;
//     }, ENCODING_RESTART_PAUSE_MS);
// }

/**
 * Handles the exit event of an FFmpeg process, renaming the temp file.
 * @param {string} identifier Camera identifier.
 * @param {number|null} code The exit code.
 * @param {string|null} signal The signal that terminated the process.
 */
// function handleFFmpegExit(identifier, code, signal) {
//     const camera = cameras[identifier];
//     if (!camera) return;

//     console.log(`FFmpeg for ${identifier} exited with code ${code}, signal ${signal}.`);
//     const { baseTempFile, outputFile } = camera;
...
async function upsertItemDB(input, tableName) {
    ...
    try {
        ...
        for (const item of items) {
            const [rows] = await conn.query(`SELECT id FROM \`${tableName}\` WHERE title = ? LIMIT 1;`, [item.title]);
            let id;

            if (rows.length > 0) { // Update existing
                id = rows[0].id;
                ...
                summary.resultMessage.push(`Successfully updated ${item.title} in ${tableName}`);
            } else { // Insert new
                ...
                summary.resultMessage.push(`Successfully inserted ${item.title} in ${tableName}`);
            }
...
function repairJPEG(buffer) {
    if (buffer.length < 4) {
        console.warn("Buffer too small to repair as JPEG.");
        return null;
    }
...
/**
 * Generates temporary and final file names for a video recording.
 * @param {string} identifier The camera identifier.
 * @returns {{baseTempFile: string, outputFile: string}} The generated file paths.
 */
function generateFileNames(identifier) {
    ...
    const baseTempFile = path.join(activePath, `.temp_${identifier}.mp4`);
    const outputFile = path.join(activePath, `${identifier}_${formattedDateTime}.mp4`);
...
async function refreshTemperatureBuffer() {
    try {
        ...
        console.error("Failed to refresh temperature buffer:", error);
    }
}
...
async function shutdownCamera(identifier) {
    ...
    const { ffmpegProcess, baseTempFile, outputFile, stream } = cam;
    console.log(`Shutting down camera ${identifier}...`);
